...
typedef struct AuthCompatConn AuthCompatConn;
typedef struct AuthCompatProto AuthCompatProto;
typedef struct AuthCompatStream AuthCompatStream;

static AuthCompatStream *auth_compat_stream_new(struct AuthCompatConn *conn,
    legacy_security_stream_t *leg_stream, auth_state_t state);
static AuthCompatProto *auth_compat_proto_new(struct AuthCompatConn *conn,
    auth_state_t state);

/*
 * Legacy quirk handling
 *
 * TODO: not really necessary at this point?
 */
...
struct AuthCompatConn {
    ...
    /* an outstanding callback, and its argument */
...
compat_listen_callback(
    ...
    pkt_t *pkt)
{
    AuthCompatConn *self = listen_info.conn;
    AuthCompatProto *proto;

    AUTH_DEBUG(6, "compat_listen_callback(%p, %p)", handle, pkt);

    /* If this object does not yet have a legacy_security_handle, well, now it
     * does.  If it already had a handle, this had best be the *same* handle. */
    /* TODO: state tracking: NEW to CONNECTED?? */
    g_assert(handle != NULL);
    if (!self->leg_handle)
	self->leg_handle = handle;
    g_assert(self->leg_handle == handle);

    /* A NULL packet indicates that we will not receive any more protocol
     * communications here (and thus that amandad may want to shut down soon).
     * This gets sent directly to the proto_cb. */
    if (!pkt) {
	AUTH_DEBUG(2, "compat_listen_callback: got NULL packet; sending NULL to proto_cb");
	listen_info.cb(listen_info.cb_arg, AUTH_CONN(self), NULL);
	return;
    }

    /* The legacy API will call this callback every time an incoming packet
     * arrives and the previous protocol transaction has been finished.  That
     * means that it's time to create a new proto object, pre-primed with the
     * enclosed packet. */
    proto = auth_compat_proto_new(self, AUTH_CONNECTED);
    proto->cached_pkt = pkt;
    listen_info.cb(listen_info.cb_arg, AUTH_CONN(self), AUTH_PROTO(proto));
}
    ...
    char *errmsg)
{
    ...
    return auth_compat_proto_new(self, AUTH_NEW);
}
    ...
    AuthCompatConnClass * selfc)
{
    ...
    klass->proto_new = conn_proto_new_impl;
...
static GType auth_compat_proto_get_type(void);
#define AUTH_COMPAT_PROTO_TYPE (auth_compat_proto_get_type())
#define AUTH_COMPAT_PROTO(obj) G_TYPE_CHECK_INSTANCE_CAST((obj), auth_compat_proto_get_type(), AuthCompatProto)
#define AUTH_COMPAT_PROTO_CONST(obj) G_TYPE_CHECK_INSTANCE_CAST((obj), auth_compat_proto_get_type(), AuthCompatProto const)
#define AUTH_COMPAT_PROTO_CLASS(klass) G_TYPE_CHECK_CLASS_CAST((klass), auth_compat_proto_get_type(), AuthCompatProtoClass)
#define IS_AUTH_COMPAT_PROTO(obj) G_TYPE_CHECK_INSTANCE_TYPE((obj), auth_compat_proto_get_type ())
#define AUTH_COMPAT_PROTO_GET_CLASS(obj) G_TYPE_INSTANCE_GET_CLASS((obj), auth_compat_proto_get_type(), AuthCompatProtoClass)

static GObjectClass *parent_proto_class = NULL;

struct AuthCompatProto {
    AuthProto __parent__;

    /* tracking for the state of this proto; this just helps ensure that the
     * methods are called in the correct order.  In this case,
     *  AUTH_CONNECTING = proto on a new listening connection (auth_listen), still
     *	    waiting to receive the first packet
     *  AUTH_NEW = proto from proto_new, so ready to send, but do not receive
     *  AUTH_CONNECTED = connected - send and receive at will */
    /* TODO: ^^ that's bogus. */
    auth_state_t state;

    /* Proto objects are created anew by the legacy Security API when the first
     * packet arrives, which is *before* the first call to recvpkt.  So that packet
     * gets stuck here until the recvpkt call comes in. */
    pkt_t *cached_pkt;
...
typedef struct {
    AuthProtoClass __parent__;
} AuthCompatProtoClass;

static AuthCompatProto *
auth_compat_proto_new(
    struct AuthCompatConn *conn,
    auth_state_t state)
{
    AuthCompatProto *self = AUTH_COMPAT_PROTO(g_object_new(AUTH_COMPAT_PROTO_TYPE, NULL));
    auth_proto_construct(AUTH_PROTO(self), AUTH_CONN(conn));

    AUTH_PROTO(self)->state = state;
...
proto_init_impl(
    AuthProto *proto)
{
    AuthCompatProto *self = AUTH_COMPAT_PROTO(proto);
...
proto_finalize_impl(
    GObject * obj_self)
{
    AuthCompatProto *self = AUTH_COMPAT_PROTO(obj_self);
...
proto_error_message_impl(
    AuthProto *proto)
{
    return auth_conn_error_message(proto->conn);
}
...
{
    AuthCompatProto *self = AUTH_COMPAT_PROTO(proto);
    AuthCompatConn *conn = AUTH_COMPAT_CONN(proto->conn);

    g_assert(conn->state == AUTH_CONNECTED);

    if (packet == NULL) {
	/* if packet is NULL, that means the transaction is over; this is signaled
	 * to legacy drivers using a special hack.. */
	conn->leg_handle->driver->close_packet_stream(self->leg_handle);
	self->state = AUTH_CLOSED; /* TODO: <-- fix */
	return TRUE;
    }

    return 0 == legacy_security_sendpkt(conn->leg_handle, packet);
}
...
compat_recvpkt_callback(
    ...
    legacy_security_status_t status)
{
    AuthCompatProto *self = AUTH_COMPAT_PROTO(arg);
    AuthCompatConn *conn = self->conn;
    auth_recvpkt_callback_t cb;

    AUTH_DEBUG(6, "compat_recvpkt_callback(%p, %p, %d)", arg, pkt, status);

    /* this callback should never be called when there are packets cached */
    g_assert(self->cached_pkt == NULL);
    ...
    /* note that legacy_security and auth statuses have the same values */
    cb(self->recvpkt_cb_arg, AUTH_PROTO(self), pkt, (auth_operation_status_t)status);
}
    ...
    int timeout)
{
    ...
    /* if we already have a packet, go ahead and call the callback with it now */
    if (self->cached_pkt) {
	pkt_t *pkt = self->cached_pkt;
	self->cached_pkt = NULL;

	recvpkt_cb(recvpkt_cb_arg, proto, pkt, AUTH_OK);
	return;
    }

    /* otherwise, schedule a recvpkt call */
    legacy_security_recvpkt(conn->leg_handle, compat_recvpkt_callback,
...
static void
proto_class_init(
    AuthCompatProtoClass * selfc)
{
    AuthProtoClass *klass = AUTH_PROTO_CLASS(selfc);
    GObjectClass *goc = G_OBJECT_CLASS(selfc);

    goc->finalize = proto_finalize_impl;

    klass->error_message = proto_error_message_impl;
    klass->sendpkt = proto_sendpkt_impl;
    klass->recvpkt = proto_recvpkt_impl;

    parent_proto_class = g_type_class_peek_parent(selfc);
}

static GType
auth_compat_proto_get_type (void)
{
    static GType type = 0;

    if G_UNLIKELY(type == 0) {
        static const GTypeInfo info = {
            sizeof (AuthCompatProtoClass),
            (GBaseInitFunc) NULL,
            (GBaseFinalizeFunc) NULL,
            (GClassInitFunc) proto_class_init,
            (GClassFinalizeFunc) NULL,
            NULL /* class_data */,
            sizeof (AuthCompatProto),
            0 /* n_preallocs */,
            (GInstanceInitFunc) proto_init_impl,
            NULL
        };

        type = g_type_register_static (AUTH_PROTO_TYPE, "AuthCompatProto", &info, 0);
    }
