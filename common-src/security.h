...
/*
 * This is a type that gets passed to the legacy_security_recvpkt() and
...
typedef enum {
    ...
    S_TIMEOUT,	/* no pkt_t was received within the time specified in the
		 * timeout argument to legacy_security_recvpkt() */
 ...
 * mask this.
 */
typedef struct legacy_security_driver {

    /*
     * The name of this driver, eg, "BSD", "BSDTCP", "KRB5", etc...  This is
     * used by legacy_security_getdriver() to associate a name with a driver type.
     */
    const char *name;

    /*
     * This is the implementation of legacy_security_connect(). It actually sets up
     * the connection, and then returns a structure describing the connection.
     * The first element of this structure MUST be a legacy_security_handle_t, because
     ...
     * is a legacy_security_status_t flag indicating the success or failure of the
     * operation.
     */
    void (*connect)(const char *, char *(*)(char *, void *),
	    void (*)(void *, struct legacy_security_handle *, legacy_security_status_t),
	    void *, void *);
     ...
     */
    void (*accept)(const struct legacy_security_driver *, char *(*)(char *, void *),
	    int, int, void (*)(struct legacy_security_handle *, pkt_t *), void *);

    /* get the remote hostname */
    char *(*get_authenticated_peer_name)(struct legacy_security_handle *handle);
     ...
     */
    void (*recvpkt)(void *, void (*)(void *, pkt_t *, legacy_security_status_t), void
	    *, int);
     ...
     * legacy_security_stream_t, because it will be cast to that.
     */
     ...
     * first member of this object MUST be a legacy_security_stream_t, because it will
     * be cast to that.
     */
...
} legacy_security_driver_t;

/* Given a security type ("KRB4", "BSD", "SSH", etc), returns a pointer to that
 * type's legacy_security_driver_t, or NULL if no driver exists.  */
const legacy_security_driver_t *legacy_security_getdriver(const char *);
 ...
 */
typedef struct legacy_security_handle {
    const legacy_security_driver_t *driver;
    char *error;
} legacy_security_handle_t;

/* void legacy_security_connect(
 *  const legacy_security_driver_t *driver,
 *  const char *hostname,
 ...
 *  void *datap);
 *
 * Given a security driver, and a hostname, calls back with a legacy_security_handle_t
 ...
 * via legacy_security_geterror().  The conf_fn is used to determine configuration
 * information, with its second argument being the datap. If conf_fn is NULL,
 * no configuration information is available.
 */
#define	legacy_security_connect(driver, hostname, conf_fn, fn, arg, datap)	\
    (*(driver)->connect)(hostname, conf_fn, fn, arg, datap)

/* void legacy_security_accept(
 *  const legacy_security_driver_t *driver,
 *  char *(*conf_fn)(char *, void *),
 *  int in,
 *  int out,
 ...
 * program.
 */
#define	legacy_security_accept(driver, conf_fn, in, out, fn, datap)	\
    (*(driver)->accept)(driver, conf_fn, in, out, fn, datap)

/* char *legacy_security_get_authenticated_peer_name(
 *  legacy_security_handle_t *handle);
 *
 ...
 * authenticated.
 */
#define	legacy_security_get_authenticated_peer_name(handle) \
    (*(handle)->driver->get_authenticated_peer_name)(handle)

/* Closes a security stream created by a legacy_security_connect() or
 * legacy_security_accept() and frees up resources associated with it. */
void legacy_security_close(legacy_security_handle_t *);

/* ssize_t legacy_security_sendpkt(legacy_security_handle_t *, const pkt_t *);
 *
 ...
 * legacy_security_geterror(). */
#define	legacy_security_sendpkt(handle, pkt)		\
    (*(handle)->driver->sendpkt)(handle, pkt)

/* void legacy_security_recvpkt(
 *  legacy_security_handle_t *handle,
 ...
 * legacy_security_geterror().  On successful reception, RECV_OK will be passed in the
...
#define	legacy_security_recvpkt(handle, fn, arg, timeout)	\
    (*(handle)->driver->recvpkt)(handle, fn, arg, timeout)

/* void security_recvpkt_cancel(legacy_security_handle_t *);
 *
 * Cancels a previous recvpkt request for this handle. */
#define	legacy_security_recvpkt_cancel(handle)		\
    (*(handle)->driver->recvpkt_cancel)(handle)

/* const char *legacy_security_geterror(legacy_security_handle_t *);
 *
 * Returns a descriptive error message for the last error condition on this
 * handle. */
#define	legacy_security_geterror(handle)	((handle)->error)

/* Sets the string that legacy_security_geterror() returns.  For use by security
 * drivers. */
void legacy_security_seterror(legacy_security_handle_t *, const char *, ...)
     G_GNUC_PRINTF(2,3);

/* Initializes a legacy_security_handle_t. This is meant to be called only by security
 * drivers to initialize the common part of a newly allocated
 * legacy_security_handle_t.  */
void legacy_security_handleinit(legacy_security_handle_t *, const legacy_security_driver_t *);
 ...
 */
typedef struct legacy_security_stream {
    const legacy_security_driver_t *driver;
    char *error;
} legacy_security_stream_t;

/* Initializes a legacy_security_stream_t. This is meant to be called only by security
 * drivers to initialize the common part of a newly allocated
 * legacy_security_stream_t. */
void legacy_security_streaminit(legacy_security_stream_t *, const legacy_security_driver_t *);

/* const char *legacy_security_stream_geterror(legacy_security_stream_t *);
 *
 * Returns a descriptive error message for the last error condition on this
 * stream. */
#define	legacy_security_stream_geterror(stream)	((stream)->error)

/* Sets the string that legacy_security_stream_geterror() returns. */
void legacy_security_stream_seterror(legacy_security_stream_t *, const char *, ...)
     G_GNUC_PRINTF(2,3);

/* legacy_security_stream_t *legacy_security_stream_server(legacy_security_handle_t *);
 *
 ...
 * Returns a legacy_security_stream_t on success, and NULL on error. Error messages
 * can be obtained by calling legacy_security_geterror() on the security handle
 * associated with this stream. */
#define	legacy_security_stream_server(handle)	\
    (*(handle)->driver->stream_server)(handle)

/* int legacy_security_stream_accept(legacy_security_stream_t *);
 *
 * Given a security stream created by legacy_security_stream_server, blocks until a
 ...
 * calling legacy_security_stream_geterror().
 */
#define	legacy_security_stream_accept(stream)		\
    (*(stream)->driver->stream_accept)(stream)

/* legacy_security_stream_t *legacy_security_stream_client(legacy_security_handle_t *, int);
 *
 ...
 * legacy_security_stream_t on success, and NULL on error. Error messages can be
 * obtained by calling legacy_security_geterror() on the security handle associated
 * with this stream. */
#define	legacy_security_stream_client(handle, id)	\
    (*(handle)->driver->stream_client)(handle, id)

/* Closes a security stream and frees up resources associated with it. */
void legacy_security_stream_close(legacy_security_stream_t *);

/* int legacy_security_stream_auth(legacy_security_stream_t *);
 *
 * Authenticate a connected security stream.  This should be called by the
 * target after legacy_security_stream_accept returns successfully, and by the client
 ...
 * legacy_security_stream_geterror().
 */
#define	legacy_security_stream_auth(stream)		\
    (*(stream)->driver->stream_auth)(stream)

/* int security_stream_id(legacy_security_stream_t *);
 *
 * Returns an identifier which can be used to connect to this security stream
 * with legacy_security_stream_client().  Typical usage is for one end of a connection
 * to create a stream with legacy_security_stream_server(), and then transmit the id
 * for that stream to the other side. The other side will then connect to that
 * id with legacy_security_stream_client(). */
#define	legacy_security_stream_id(stream)		\
    (*(stream)->driver->stream_id)(stream)

/* int legacy_security_stream_write(legacy_security_stream_t *, const void *, size_t);
 *
 ...
 * legacy_security_stream_geterror().
 */
#define	legacy_security_stream_write(stream, buf, size)	\
    (*(stream)->driver->stream_write)(stream, buf, size)

/* void legacy_security_stream_read(
 *  legacy_security_stream_t *stream,
 ...
 * be retrieved by calling legacy_security_stream_geterror().  This function uses the
 * event interface. Callbacks will only be generated while in event_loop(). */
#define	legacy_security_stream_read(stream, fn, arg)		\
    (*(stream)->driver->stream_read)(stream, fn, arg)

/* void legacy_security_stream_read_sync(legacy_security_stream_t *, void **);
 *
 ...
 * legacy_security_stream_geterror(). This function uses the event interface.  */
#define	legacy_security_stream_read_sync(stream, buf)		\
    (*(stream)->driver->stream_read_sync)(stream, buf)

/* void legacy_security_stream_read_cancel(legacy_security_stream_t *);
 *
 * Cancels a previous read request. */
#define	legacy_security_stream_read_cancel(stream)		\
    (*(stream)->driver->stream_read_cancel)(stream)

/* void legacy_security_close_connection(legacy_security_handle_t *, hostname *);
 *
 * Close a security handle, freeing associated resources.  The hostname
 * argument is ignored. */
#define legacy_security_close_connection(handle, hostname) \
