...
<<<<<<< HEAD
	if (rc->driver->data_read == NULL) {
	    read_bytes = read(fd, ((char *)&rc->netint) + rc->size_header_read, SIZEOF(rc->netint) - rc->size_header_read);
	} else {
	    read_bytes = rc->driver->data_read(rc, ((char *)&rc->netint) + rc->size_header_read, SIZEOF(rc->netint) - rc->size_header_read);
	}
=======
	read_bytes = rc->driver->data_read(rc, &rc->netint + rc->size_header_read, SIZEOF(rc->netint) - rc->size_header_read);
>>>>>>> 772aa8b... ssl don't use a priviledge port
	switch (read_bytes) {
	case -1:
	    if (errmsg)
